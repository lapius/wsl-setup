...
if ! command -v zsh &>/dev/null; then
  pkg update && pkg install zsh -y
fi

# Set zsh as default shell
if [ "$SHELL" != "$(which zsh)" ]; then
  chsh -s zsh 2>/dev/null || echo "export SHELL=zsh" >> ~/.bashrc
fi
...
if ! grep -q "zsh-autosuggestions" ~/.zshrc; then
  sed -i 's/plugins=(git)/plugins=(git sudo z zsh-autosuggestions zsh-syntax-highlighting)/' ~/.zshrc
fi
...
echo ""
echo "✅ Done! Please:"
echo "  1. Close and reopen Termux"
